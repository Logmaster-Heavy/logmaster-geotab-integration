...
describe('User visits addin', () => {
   ...
   // Confirm page displaying after initialized and focus is called
    it('should display root div', async () => {
        ...
        await page.waitForSelector('#logmasterEwd2', {
            visible: true
        });
    ...
    it('blur button should blur addin', async () => {
        ...
        let hidden = await page.evaluate( () => {
            let toggled = false;
            let addin = document.getElementById('logmasterEwd2');
    ...
    it('should refocus after group filter is selected', async () => {
        let originalFocus = await page.evaluate( () => geotab.addin.logmasterEwd2.focus);

        await page.click('#group-toggle-button');
        await page.evaluate( () => {
            geotab.addin.logmasterEwd2.count = 0;
            geotab.addin.logmasterEwd2.focus = () => { geotab.addin.logmasterEwd2.count++; };
        });
        await page.click('#group-use-b4-label');

        let invocations = await page.evaluate( () => {
            return geotab.addin.logmasterEwd2.count;
        });
        ...
        await page.evaluate( (addinFocus) => {
            geotab.addin.logmasterEwd2.focus = addinFocus;
        }, originalFocus);
    });
